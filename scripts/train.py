...
기존 단일 GPU 학습 코드를 Accelerate 기반 멀티 GPU(DDP) 학습으로 확장했습니다.
...
- validation metric 계산을 위해 embedding을 gather하여 전체 프로세스 기준으로 계산합니다.
"""
...
def reshape_negatives(flat_embeds: torch.Tensor, counts: list[int]) -> torch.Tensor | None:
...
def train_one_epoch(
    ...
    for batch in pbar:
        ...
        pos_ids = batch["positive_input_ids"]
        pos_mask = batch["positive_attention_mask"]
        ...
        if "hard_neg_input_ids" in batch:
            hn_ids = batch["hard_neg_input_ids"]
            hn_mask = batch["hard_neg_attention_mask"]
        ...
        if "neg_input_ids" in batch:
            n_ids = batch["neg_input_ids"]
            n_mask = batch["neg_attention_mask"]
        ...
        total_loss += loss_dict["loss"].detach().float().item()
        total_infonce += loss_dict.get("infonce_loss", torch.tensor(0.0, device=loss.device)).detach().float().item()
        total_triplet += loss_dict.get(
            "triplet_loss",
            loss_dict.get("hard_margin_loss", torch.tensor(0.0, device=loss.device)),
        ).detach().float().item()
        num_batches += 1
...
@torch.no_grad()
def evaluate(model, dataloader, loss_fn, accelerator: Accelerator) -> dict:
    model.eval()
    total_loss = 0.0
    num_batches = 0
    ...
    for batch in tqdm(dataloader, desc="Evaluating", disable=not accelerator.is_local_main_process):
        ...
        gathered_q = accelerator.gather_for_metrics(query_embeds)
        gathered_p = accelerator.gather_for_metrics(positive_embeds)
        all_query_embeds.append(gathered_q.cpu())
        all_positive_embeds.append(gathered_p.cpu())
...
def compute_ndcg(ranks: torch.Tensor, k: int = 10) -> float:
    ...
    return dcg / (len(ranks) * ideal_dcg) if ideal_dcg > 0 else 0.0
...
def main():
    ...
    accelerator = Accelerator(
        mixed_precision="fp16" if train_config.get("fp16", False) else "no",
    )
    ...
    total_steps = len(train_loader) * train_config["epochs"] // max(
        int(train_config.get("gradient_accumulation_steps", 1)), 1
    )
    ...
    for epoch in range(train_config["epochs"]):
        ...
        val_metrics = evaluate(model, val_loader, loss_fn, accelerator)
        ...
        if val_metrics["mrr"] > best_mrr:
            ...
            if accelerator.is_local_main_process:
                print("  => New best MRR! Saving model...")
